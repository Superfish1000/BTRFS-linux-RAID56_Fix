...
scrub)
	...
	;;
r6deg_prep)
	# open/r6deg: RAID6 over five devices, a nodatacow file and a checksummed
	# one, and where the first whole full stripe of the nodatacow file lives.
	watchdog 600
	dm_setup
	mkfs.btrfs -K -q -f -d $DPROF -m $MPROF $DMDEVS || { log "MKFS_FAIL"; finish; }
	dm_scan
	...
	allow_nodatacow
	mkdir -p $MNT/nc $MNT/c; chattr +C $MNT/nc
	dd if=/dev/urandom of=$MNT/nc/f bs=64K count=48 conv=fsync status=none
	dd if=/dev/urandom of=$MNT/c/f bs=64K count=48 conv=fsync status=none
	sync
	cp $MNT/nc/f $T/umltest/r6.orig.$TAG
	md5sum $MNT/c/f | awk '{print $1}' > $T/umltest/r6.cmd5.$TAG
	lsattr $MNT/nc/f | while read -r l; do log "lsattr $l"; done
	python3 $T/umltest/r6deg_layout.py $MNT/nc/f /dev/mapper/d0 > $T/umltest/r6.layout.$TAG 2>&1
	log "layout $(cat $T/umltest/r6.layout.$TAG)"
	umount $MNT || log "UMOUNT_FAIL"
	dmsetup remove_all
	finish
	;;
r6deg_fault)
	# Degraded (the host omitted OMITTED), or healthy with Q failing writes
	# (VARIANT=healthy).  Overwrite ONE 4K block of data column 0 (device X)
	# in place while X fails writes: acknowledged within RAID6's tolerance,
	# X is recorded stale and so is Q (missing, or failing).  The repair the
	# fault queues runs one second later with X still failing: its phase A
	# write-back fails.  Retries are pushed out of the way, X is healed and
	# the log committed.  The host then reads with every device present.
	dm_setup
	dm_scan
	watchdog 600
	dmi() { local k=$1; [ -n "${OMITTED:-}" ] && [ "$k" -gt "$OMITTED" ] && k=$((k-1)); echo $k; }
	XI=$(dmi $X)
	if [ "${VARIANT:-degraded}" = healthy ]; then
		do_mount rw /dev/mapper/d0
	else
		do_mount rw,degraded /dev/mapper/d0
	fi
	allow_nodatacow
	stats before
	echo ${DELAY1:-1000} > /sys/module/btrfs/parameters/raid56_repair_delay_ms
	head -c 4096 /dev/urandom > $T/umltest/r6.new.$TAG
	dm_error_writes $XI; log "write errors on device $X (dm d$XI)"
	if [ "${VARIANT:-degraded}" = healthy ]; then
		QI=$(dmi $QDEV); dm_error_writes $QI; log "write errors on Q device $QDEV (dm d$QI)"
	fi
	rm -f $T/umltest/r6.acked.$TAG
	if dd if=$T/umltest/r6.new.$TAG of=$MNT/nc/f bs=4096 count=1 seek=$((FO0 / 4096)) \
		conv=notrunc,fsync status=none; then
		echo 1 > $T/umltest/r6.acked.$TAG; log "OVERWRITE_ACKED"
	else
		log "OVERWRITE_REFUSED"
	fi
	# Keep every retry out of the way: the first attempt was queued with
	# DELAY1 already, the retries read the knob when they are queued.
	echo 600000 > /sys/module/btrfs/parameters/raid56_repair_delay_ms
	if [ "${VARIANT:-degraded}" = healthy ]; then dm_heal $QI; log "healed Q device $QDEV"; fi
	for i in $(seq 1 40); do
		f=$(sed -n 's/^repair_failed //p' /sys/fs/btrfs/*/raid56_write_intent)
		o=$(sed -n 's/^repair_ok //p' /sys/fs/btrfs/*/raid56_write_intent)
		[ "${f:-0}" -gt 0 ] || [ "${o:-0}" -gt 0 ] && break
		sleep 0.5
	done
	stats "after repair attempt"
	kmsg "raid56" 12
	dm_heal $XI; log "healed device $X"
	sync; sleep 1; sync
	echo 3 > /proc/sys/vm/drop_caches
	R=$(dd if=$MNT/nc/f bs=4096 count=1 skip=$((FO0 / 4096)) status=none 2>/dev/null | md5sum | awk '{print $1}')
	N=$(md5sum < $T/umltest/r6.new.$TAG | awk '{print $1}')
	O=$(dd if=$T/umltest/r6.orig.$TAG bs=4096 count=1 skip=$((FO0 / 4096)) status=none | md5sum | awk '{print $1}')
	if [ "$R" = "$N" ]; then log "IN_MOUNT_READ new"
	elif [ "$R" = "$O" ]; then log "IN_MOUNT_READ OLD"
	else log "IN_MOUNT_READ other/EIO ($R)"; fi
	stats end
	umount $MNT || log "UMOUNT_FAIL"
	dmsetup remove_all
	finish
	;;
r6deg_check)
	# Every device back.  CHECK=ro: -o ro, no recovery runs, the record read
	# off the disks is what the reads consult.  CHECK=scrub: rw (the mount
	# recovery runs), then a full scrub, then the same checks.
	dm_setup
	dm_scan
	watchdog 600
	if [ "${CHECK:-ro}" = ro ]; then
		...
		do_mount rw /dev/mapper/d0
		stats "after mount"
		kmsg "raid56 write-intent log|scrub" 8
		btrfs scrub start -B $MNT 2>&1 | grep -E "Error summary|corrected|uncorrectable|unverified|no errors" |
			while read -r l; do log "scrub: $l"; done
		stats "after scrub"
	fi
	echo 3 > /proc/sys/vm/drop_caches
	V=$(python3 - "$MNT/nc/f" "$T/umltest" "$TAG" "$FO0" <<'PY'
import os, sys
path, d, tag, fo0 = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4])
BS = 4096
orig = open(f'{d}/r6.orig.{tag}', 'rb').read()
new = open(f'{d}/r6.new.{tag}', 'rb').read()
acked = os.path.exists(f'{d}/r6.acked.{tag}')
fd = os.open(path, os.O_RDONLY)
res = {'ok': 0, 'eio': [], 'silent': []}
target = None
for b in range(len(orig) // BS):
    try:
        blk = os.pread(fd, BS, b * BS)
    except OSError:
        res['eio'].append(b)
        if b * BS == fo0:
            target = 'EIO'
        continue
    old = orig[b*BS:(b+1)*BS]
    if b * BS == fo0:
        if blk == new:
            target = 'NEW'
        elif blk == old:
            ...
            target = 'GARBAGE'
        ok = blk == new if acked else blk in (old, new)
    else:
        ok = blk == old
    if ok:
        res['ok'] += 1
        ...
        res['silent'].append(b)
print(f"target={target} acked={int(acked)} ok={res['ok']} eio={len(res['eio'])} "
      f"silent={len(res['silent'])} eio_blocks={res['eio'][:8]} silent_blocks={res['silent'][:8]}")
PY
)
	log "NOCOW $V"
	C=$(md5sum $MNT/c/f 2>/dev/null | awk '{print $1}')
	[ "$C" = "$(cat $T/umltest/r6.cmd5.$TAG)" ] && log "CSUMFILE ok" || log "CSUMFILE BAD (${C:-READFAIL})"
	kmsg "raid56|csum|Q syndrome|rebuilt from parity" 10
	stats end
	umount $MNT || log "UMOUNT_FAIL"
	dmsetup remove_all
	finish
	;;
r6degL_prep)
	# open/r6deg literal scenario, boot 1 (healthy): files, layout, then one
	# device (X = column 0 of stripe 0) fails writes briefly under in-place
	# overwrites of one 4K block of column 0 in each of the first stripes.
	# The queued repair is off (raid56_no_repair_on_fault=1 from the host), so
	# the stale columns survive to the next mount.
	watchdog 600
	dm_setup
	mkfs.btrfs -K -q -f -d $DPROF -m $MPROF $DMDEVS || { log "MKFS_FAIL"; finish; }
	dm_scan
	...
	cp $MNT/nc/f $T/umltest/r6.orig.$TAG
	md5sum $MNT/c/f | awk '{print $1}' > $T/umltest/r6.cmd5.$TAG
	rm -f $T/umltest/r6.exp.$TAG
	python3 $T/umltest/r6deg_layout.py $MNT/nc/f /dev/mapper/d0 ${KSTRIPES:-8} > $T/umltest/r6.layout.$TAG 2>&1
	log "layout $(cat $T/umltest/r6.layout.$TAG)"
	eval "$(cat $T/umltest/r6.layout.$TAG)"
	dm_error_writes $D0; log "write errors on device $D0"
	for k in $(seq 0 $((NSTRIPES-1))); do
		eval "s=\$S$k"; fo=${s%%:*}
		head -c 4096 /dev/urandom > $T/umltest/r6.blk.$TAG
		if dd if=$T/umltest/r6.blk.$TAG of=$MNT/nc/f bs=4096 count=1 seek=$((fo / 4096)) \
			conv=notrunc,fsync status=none 2>/dev/null; then a=1; else a=0; fi
		python3 $T/umltest/r6exp.py record $T/umltest/r6.exp.$TAG $fo $T/umltest/r6.blk.$TAG $a
		log "prep overwrite stripe $k ($s) acked=$a"
	done
	dm_heal $D0; log "healed device $D0"
	stats "after prep faults"
	umount $MNT || log "UMOUNT_FAIL"
	dmsetup remove_all
	finish
	;;
r6degL_degraded)
	# Boot 2: a DIFFERENT device (OMITTED) missing.  The mount recovery runs
	# first.  Then the Q device of stripe 0 (QDEV) fails writes while one 4K
	# block of column 1 of every stripe is overwritten in place, and the
	# queued repairs run (raid56_repair_delay_ms=1000).  QDEV is healed, the
	# repairs are given time, and the filesystem is unmounted.
	dm_setup
	dm_scan
	watchdog 600
	dmi() { local k=$1; [ -n "${OMITTED:-}" ] && [ "$k" -gt "$OMITTED" ] && k=$((k-1)); echo $k; }
	do_mount rw,degraded /dev/mapper/d0
	allow_nodatacow
	stats "after degraded mount"
	kmsg "raid56 write-intent log|scrub:" 8
	eval "$(cat $T/umltest/r6.layout.$TAG)"
	echo 1000 > /sys/module/btrfs/parameters/raid56_repair_delay_ms
	QI=$(dmi $QDEV); dm_error_writes $QI; log "write errors on device $QDEV (dm d$QI)"
	for k in $(seq 0 $((NSTRIPES-1))); do
		eval "s=\$S$k"; fo=$(( ${s%%:*} + 65536 ))
		head -c 4096 /dev/urandom > $T/umltest/r6.blk.$TAG
		if dd if=$T/umltest/r6.blk.$TAG of=$MNT/nc/f bs=4096 count=1 seek=$((fo / 4096)) \
			conv=notrunc,fsync status=none 2>/dev/null; then a=1; else a=0; fi
		python3 $T/umltest/r6exp.py record $T/umltest/r6.exp.$TAG $fo $T/umltest/r6.blk.$TAG $a
		log "degraded overwrite stripe $k col 1 ($s) acked=$a"
	done
	sleep ${SETTLE1:-12}
	stats "while $QDEV fails"
	kmsg "raid56" 14
	dm_heal $QI; log "healed device $QDEV"
	sleep ${SETTLE2:-40}
	stats "after heal"
	kmsg "raid56: (gave up|could not|refusing)" 6
	V=$(python3 $T/umltest/r6exp.py check $T/umltest/r6.exp.$TAG $MNT/nc/f $T/umltest/r6.orig.$TAG)
	log "DEGRADED_READ $V"
	umount $MNT || log "UMOUNT_FAIL"
	dmsetup remove_all
	finish
	;;
r6degL_check)
	# Boot 3: every device back, rw (the mount recovery runs), scrub, check
	...
	;;
r6degF2_fault)
	# open/r6deg F2: the repair's own FLUSH fails.  Q of the stripe missing
	# (the host omitted it).  'B' into column 0 while X (column 0's device)
	# fails writes: acknowledged, X recorded stale.  X healed; then a write
	# into column 2 while W (column 1's device) fails writes: phase A puts
	# 'B' back on X, clears X's mark in memory, and btrfs_wib_persist_now()
	# fails on W's flush -- the write is refused.  W healed.  NOFLUSHFAIL=1
	# (the control) skips W's failure, so the clear becomes durable.  Then
	# a write into column 2 with raid56_crash_point=2: its data is dropped,
	# P lands, the kernel panics.  The next boot reads 'B'.
	dm_setup
	dm_scan
	watchdog 600
	dmi() { local k=$1; [ -n "${OMITTED:-}" ] && [ "$k" -gt "$OMITTED" ] && k=$((k-1)); echo $k; }
	...
	allow_nodatacow
	echo 600000 > /sys/module/btrfs/parameters/raid56_repair_delay_ms
	rm -f $T/umltest/r6.exp.$TAG $T/umltest/r6.acked.$TAG
	FO2=$((FO0 + 131072))
	w() {	# offset label -> records acked 0/1
		head -c 4096 /dev/urandom > $T/umltest/r6.blk.$TAG
		if dd if=$T/umltest/r6.blk.$TAG of=$MNT/nc/f bs=4096 count=1 seek=$(($1 / 4096)) \
			conv=notrunc,fsync status=none 2>/dev/null; then a=1; else a=0; fi
		python3 $T/umltest/r6exp.py record $T/umltest/r6.exp.$TAG $1 $T/umltest/r6.blk.$TAG ${2:-$a}
		log "write $1 acked=$a"
	}
	dm_error_writes $XI; log "write errors on X=$X"
	w $FO0
	cp $T/umltest/r6.blk.$TAG $T/umltest/r6.new.$TAG
	dm_heal $XI; log "healed X"
	if [ "${NOFLUSHFAIL:-0}" != 1 ]; then dm_error_writes $WI; log "write errors on W=$W"; fi
	w $FO2
	dm_heal $WI; log "healed W"
	stats "before crash"
	kmsg "raid56: (could not|refusing)" 4
	echo 2 > /sys/module/btrfs/parameters/raid56_crash_point
	log "crash armed"
	# Not acknowledged whatever happens: the kernel is meant to panic in it.
	w $FO2 0
	log "NO_CRASH"
	finish
	;;
r6degF2_check)
	# After the crash: ro (DEGRADED=1: Q still omitted), no recovery; the
	# record read off the disks decides.  Then the target block and every
	# other block are checked.
	dm_setup
	dm_scan
	watchdog 600
	do_mount "ro${DEGRADED:+,degraded}" /dev/mapper/d0
	echo 3 > /proc/sys/vm/drop_caches
	N=$(md5sum < $T/umltest/r6.new.$TAG | awk '{print $1}')
	O=$(dd if=$T/umltest/r6.orig.$TAG bs=4096 count=1 skip=$((FO0 / 4096)) status=none | md5sum | awk '{print $1}')
	R=$(dd if=$MNT/nc/f bs=4096 count=1 skip=$((FO0 / 4096)) status=none 2>/dev/null | md5sum | awk '{print $1}')
	if dd if=$MNT/nc/f bs=4096 count=1 skip=$((FO0 / 4096)) status=none >/dev/null 2>&1; then
		if [ "$R" = "$N" ]; then log "TARGET NEW"; elif [ "$R" = "$O" ]; then log "TARGET OLD"; else log "TARGET GARBAGE"; fi
	else
		log "TARGET EIO"
	fi
	V=$(python3 $T/umltest/r6exp.py check $T/umltest/r6.exp.$TAG $MNT/nc/f $T/umltest/r6.orig.$TAG)
	log "NOCOW_ALL $V"
	kmsg "raid56|Q syndrome|rebuilt from parity" 8
	umount $MNT || log "UMOUNT_FAIL"
	dmsetup remove_all
	finish
	;;
r6degF1b_fault)
	# open/r6deg F1b: a refused rbio clears the stale mark of a MISSING data
	# column it was given but never wrote.  Column 1's device (Y) omitted by
	# the host.  'B' into column 1 (acknowledged: only Y missing; Y recorded
	# stale).  'C' into column 0 while X (column 0's device) fails writes
	# (acknowledged: Y missing + X; X stale).  Then a whole-column (64 KiB)
	# write into column 1 while X still fails: phase A cannot put X back,
	# the write is refused -- and rmw_update_stale_data() sees column 1
	# "supplied" with no error bit and clears Y's mark.  CONTROL=heal heals
	# X first (phase A lands, nothing refused).
	dm_setup
	dm_scan
	watchdog 600
	dmi() { local k=$1; [ -n "${OMITTED:-}" ] && [ "$k" -gt "$OMITTED" ] && k=$((k-1)); echo $k; }
	...
	rm -f $T/umltest/r6.exp.$TAG
	FO1=$((FO0 + 65536))
	w() {	# offset bytes -> acked 0/1 recorded per 4K block
		head -c $2 /dev/urandom > $T/umltest/r6.blk.$TAG
		if dd if=$T/umltest/r6.blk.$TAG of=$MNT/nc/f bs=$2 count=1 oflag=seek_bytes seek=$1 \
			conv=notrunc,fsync status=none 2>/dev/null; then a=1; else a=0; fi
		for o in $(seq 0 4096 $(($2 - 4096))); do
			dd if=$T/umltest/r6.blk.$TAG of=$T/umltest/r6.one.$TAG bs=4096 count=1 skip=$((o / 4096)) status=none
			python3 $T/umltest/r6exp.py record $T/umltest/r6.exp.$TAG $(($1 + o)) $T/umltest/r6.one.$TAG $a
		done
		log "write $1 +$2 acked=$a"
	}
	w $FO1 4096
	cp $T/umltest/r6.blk.$TAG $T/umltest/r6.new.$TAG
	dm_error_writes $XI; log "write errors on X=$X"
	w $FO0 4096
	[ "${CONTROL:-}" = heal ] && { dm_heal $XI; log "healed X (control)"; }
	w $FO1 65536
	dm_heal $XI; log "healed X"
	stats end
	kmsg "raid56: (could not|refusing)" 4
